...
- 人审兜底、异常告警与回滚
- 自动化运营的监控与SOP
- T7 测试店授权前置包(CLI/AI Toolkit/Dev MCP 本地 preflight、测试店创建、人审文本)
...
T7 当前先走前置包:本地运行 `python3 kb/tools/t7_test_store_preflight.py --store-domain your-dev-store.myshopify.com`,确认 Node/Git/npm/npx、插件资产、Runbook 与测试店域名格式;随后由用户现场完成 Shopify 测试店创建/登录授权。前置包通过前,不进入 Dev MCP 只读连接或 mutation 预案。
