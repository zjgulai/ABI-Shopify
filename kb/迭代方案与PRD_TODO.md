...
type: plan
updated: 2026-06-26
summary: 盘点未完成事项,给出每项高质量执行方案;含「每节点 主-副工具 × 实现方法」PRD 核心表;并制定可勾选 TODO 与里程碑。
---
...
| 编号 | 事项 | 现状 |
|---|---|---|
| T1 | 商业图堆叠修复 | 架构图「商业价值」条过宽压到「人审/合规闸」竖栏;全景图底部信息源标签压块。修复脚本已就绪,**尚未生效**(命令格式错) |
| T2 | 网站迭代 | 节点详情未列「专题文档」;「下一步」仍旧四阶段;页脚仍「三源」;`kb_data` 未渲染 node.docs |
| T3 | 完整 PRD | 每节点主-副工具/实现方法未在图上完整体现,需独立 PRD(核心表见 §C) |
| T4 | 检索生产化 | LSA 离线嵌入待换 bge-m3/OpenAI;向量库/图库(Chroma/Qdrant + Neo4j)未接 |
| T5 | 网站上线 | 腾讯云部署包就绪,DeepSeek API Key 改为网站页面手动录入 |
| T6 | 多源深挖 | 视频仅编目,内容级深度萃取需字幕;抖音/小红书待补 |
...
- 验收:点节点能看到并打开其专题文档;下一步/页脚为最新;站点统计 268/213/593。
...
**T4 · 检索生产化(P1,1–2 天)**
- 做法:`pip install sentence-transformers`,`retriever.LSAEmbedder`→ bge-m3;`index_store` 换 Chroma(或 Qdrant);`_kg` 导入 Neo4j;`mcp_server` 暴露 `kb_search/kb_ask`。
- 验收:中文检索召回质量明显优于 LSA;MCP 可被 Claude Code 调用。
...
**T6 · 多源深挖(P1,持续)**
- 做法:按「粘贴字幕→8 段结构深度萃取→入图谱/RAG/站点」逐条做高价值视频;抖音/小红书同「粘贴清单→归类」。
...
## E. TODO(勾选清单)
- [ ] T1 修复全部商业图堆叠(架构/全景)并逐图核验
- [ ] T2 网站迭代:节点专题文档渲染 + 下一步P0P1P2 + 页脚8源 + 重建 kb_data
- [ ] T3 输出完整 PRD 文档(基于 §C 扩写)
- [ ] T4 检索生产化(bge-m3 + Chroma/Qdrant + Neo4j + MCP)
- [ ] T5 网站上线腾讯云(配 Key,用户侧)
