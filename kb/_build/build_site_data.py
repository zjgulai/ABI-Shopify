...
configuration_center={
    ...
    "stageFlow":[
        {"id":"local-config","label":"1. 本地配置","status":"requires_user_input","readyStatus":"local_config_ready","detail":"填写测试店域名、审批人、目标和计划变更;只保存在浏览器。","evidence":"store_domain + 三条确认","nextAction":"保存本地配置"},
        {"id":"preflight","label":"2. 本地 preflight","status":"blocked_auth_preflight_ready","readyStatus":"local_evidence_ready","detail":"在本机运行只读 preflight,确认 CLI/插件/Runbook/目标店准备项。","evidence":"preflight 输出摘要","nextAction":"记录 preflight 证据"},
        {"id":"manual-auth","label":"3. 现场授权","status":"blocked_manual_auth","readyStatus":"auth_required","detail":"用户现场执行 shopify auth login,目标必须是测试店或 development store。","evidence":"whoami 或授权摘要","nextAction":"用户现场授权"},
        {"id":"read-only","label":"4. 只读复核","status":"blocked_auth","readyStatus":"read_ready","detail":"仅做 whoami、products/count、themes/list 等低风险只读核对。","evidence":"只读响应摘要","nextAction":"记录只读证据"},
        {"id":"preview-approval","label":"5. Preview + 人审","status":"approval_required","readyStatus":"preview_ready","detail":"生成 mutation preview 与回滚方案,逐次明确审批。","evidence":"preview + approval_text","nextAction":"复制审批文本"},
        {"id":"post-check","label":"6. 写后复查/回滚","status":"blocked_auth","readyStatus":"executed_test_store","detail":"只在测试店执行低风险写入后记录结果、复查和回滚/保留决定。","evidence":"post_read + rollback_record","nextAction":"记录执行结果"},
    ],
    "checklist":[
        {"id":"domain-confirmed","label":"测试店域名已确认","stage":"local-config","detail":"域名必须是 name.myshopify.com,且由用户确认是 development store。"},
        {"id":"no-secret-input","label":"密钥边界已确认","stage":"local-config","detail":"本页不输入 Shopify token/password/private key。"},
        {"id":"preflight-run","label":"本地 preflight 已运行","stage":"preflight","detail":"运行 t7_test_store_preflight.py,结果只证明本地准备状态。"},
        {"id":"auth-user-present","label":"Shopify CLI 登录由用户现场执行","stage":"manual-auth","detail":"Codex 不代替用户登录或选择店铺。"},
        {"id":"readonly-recorded","label":"只读连接证据已记录","stage":"read-only","detail":"记录 whoami 或只读查询摘要,不记录敏感数据。"},
        {"id":"preview-ready","label":"Mutation preview 已生成","stage":"preview-approval","detail":"写入前必须有目标、计划变更、预期影响和回滚方案。"},
        {"id":"approval-text","label":"逐次人审文本已确认","stage":"preview-approval","detail":"每次写入都要单独审批,不能复用旧授权。"},
        {"id":"rollback-plan","label":"写后复查/回滚方案已准备","stage":"post-check","detail":"执行后必须记录 post-read 与回滚/保留决定。"},
    ],
    "operationPlaybooks":[
        {"id":"readonly","title":"只读连接检查","status":"auth_required","inputs":["store_domain","shopify auth login","shopify auth whoami"],"steps":["用户现场登录 CLI","确认 whoami 目标店铺","只读读取 products/count 或 themes/list 摘要","写入本地证据台账"],"outputs":["readonly_connection_note"],"boundary":"只读;不读取生产敏感数据;不写店铺。"},
        {"id":"preview","title":"Mutation preview","status":"approval_required","inputs":["mutationTarget","plannedChange","rollbackPlan"],"steps":["描述计划变更","生成预期 mutation 与影响范围","写清回滚/保留口径","复制给用户审批"],"outputs":["mutation_preview","approval_text"],"boundary":"preview 不是执行;没有审批不进入写入。"},
        {"id":"controlled-write","title":"测试店受控写入","status":"blocked_auth","inputs":["test_store","approval_text","preview"],"steps":["确认目标仍是测试店","执行单次低风险 mutation","立即做 post-read 复查","记录结果和证据"],"outputs":["execution_record","post_read_record"],"boundary":"只允许测试店;禁止生产店、支付、退款、广告预算和客户隐私动作。"},
        {"id":"rollback","title":"写后复查与回滚","status":"manual_review_required","inputs":["execution_record","post_read_record"],"steps":["比对写前/写后状态","用户决定保留或回滚","执行回滚前再次确认","记录最终状态"],"outputs":["rollback_record","final_decision"],"boundary":"回滚同样需要人审;不可把执行记录写成生产变更。"},
    ],
    "commandTemplates":{
        ...
        "devMcp":"claude mcp add shopify-dev-mcp -- npx -y @shopify/dev-mcp@latest",
        "marketplacePlugin":"claude plugin install ./kb/marketplace/plugins/shopify-ai-kb",
    },
    "approvalTemplates":{
        ...
        "preview":"请先生成 {store_domain} 测试店的 mutation preview:目标={target};计划变更={planned_change};未再次审批前不得执行。",
        "rollback":"同意对 {store_domain} 测试店本次测试写入执行复查/回滚决策;禁止扩大到生产店或资金动作。",
    },
    "evidenceActions":[
        {"id":"preflight","label":"preflight 摘要","status":"local_pass_auth_required","boundary":"local-only; no Shopify login/read/write"},
        {"id":"auth_whoami","label":"现场授权 / whoami","status":"auth_user_present","boundary":"user-present auth summary only"},
        {"id":"read_only","label":"只读连接","status":"read_only_verified","boundary":"read-only test-store evidence"},
        {"id":"mutation_preview","label":"mutation preview","status":"preview_ready","boundary":"preview only; no execution"},
        {"id":"approval","label":"人审批准","status":"approval_recorded","boundary":"single-operation approval"},
        {"id":"execute","label":"测试店执行记录","status":"executed_test_store","boundary":"test-store-only write record"},
        {"id":"rollback","label":"复查/回滚记录","status":"post_check_recorded","boundary":"post-read and rollback decision"},
    ],
    "forbidden":[
...
evidence_items=[
    ...
    {"claim":"T7 授权作战台已落地,用于承接测试店域名、本地 preflight、现场授权、只读复核、mutation preview、人审文本和本地证据台账。","source":"kb/10-自动化编排/T7授权作战台PRD与执行TODO.md + kb/site/index.html","status":"local_ready","nodes":["10","91","92"]},
...
query_presets=[
    ...
    {"label":"T7 授权作战台","query":"T7 授权作战台如何管理 preflight、现场授权、只读复核、mutation preview 和人审证据?","context":["10","91","92"]},
